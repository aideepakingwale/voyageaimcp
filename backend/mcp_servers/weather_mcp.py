...
CITY_COORDS = {
    ...
    "AGP": (36.7213, -4.4213, "Malaga", "Spain"),
    ...
    "SEZ": (-4.6796, 55.4930, "Mahe", "Seychelles"),
    ...
    "VNS": (25.3176, 82.9739, "Varanasi", "India"),
}

MONTHLY_CLIMATE = {
    "LIS": [(13, 96, 4, "Pleasant")] * 12,
    "BCN": [(13, 41, 5, "Pleasant")] * 12,
    "MAD": [(9, 39, 5, "Pleasant")] * 12,
    "TFS": [(21, 30, 6, "Pleasant")] * 12,
    "FCO": [(11, 62, 4, "Pleasant")] * 12,
    "CDG": [(6, 50, 2, "Pleasant")] * 12,
    "ATH": [(12, 48, 4, "Pleasant")] * 12,
    "DXB": [(19, 10, 8, "Pleasant")] * 12,
    "SIN": [(30, 257, 6, "Pleasant")] * 12,
    "BKK": [(32, 9, 8, "Pleasant")] * 12,
    "DPS": [(29, 232, 7, "Pleasant")] * 12,
    "MLE": [(29, 114, 9, "Pleasant")] * 12,
    "SEZ": [(28, 252, 6, "Pleasant")] * 12,
    "MRU": [(27, 277, 8, "Pleasant")] * 12,
    "NBO": [(24, 68, 7, "Pleasant")] * 12,
    "CPT": [(26, 8, 11, "Pleasant")] * 12,
    "NRT": [(9, 52, 6, "Pleasant")] * 12,
    "JFK": [(3, 91, 5, "Pleasant")] * 12,
    "LAX": [(18, 84, 7, "Pleasant")] * 12,
    "SYD": [(26, 103, 7, "Pleasant")] * 12,
    "AKL": [(23, 72, 7, "Pleasant")] * 12,
    "MIA": [(24, 51, 8, "Pleasant")] * 12,
    "CUN": [(27, 45, 7, "Pleasant")] * 12,
    "HNL": [(26, 72, 8, "Pleasant")] * 12,
    "BOB": [(30, 135, 8, "Pleasant")] * 12,
    "VNS": [(35, 287, 7, "Pleasant")] * 12,
}


class WeatherMCP(BaseMCP):
    def __init__(self):
        super().__init__(ttl=1800)
        self._diag = {}

    def _fetch(self, params: dict) -> dict:
        city_code = params.get("city", "LIS").upper()[:3]
        month_idx = int(params.get("month", datetime.now().month)) - 1
        info = CITY_COORDS.get(city_code, CITY_COORDS["LIS"])
        lat, lon, city_name, country = info
        ...
        self._set_diag(
            status="fallback_climate",
            city=city_code,
            resolved_city=city_name,
            country=country,
            reason="Live OpenWeather data unavailable; using climate averages fallback.",
            auth_configured=bool(api_key),
        )
        fallback = self._climate(city_code, city_name, country, month_idx)
        fallback["provider_diagnostics"] = self._diag
        return fallback

    def _set_diag(self, **data):
        self._diag = data
    ...
    def _live(self, api_key, lat, lon, city_name, country, code, month_idx):
        try:
            ...
            wind = round(weather["wind"]["speed"] * 3.6, 1)
            icon = weather["weather"][0]["icon"]
            vis = round(weather.get("visibility", 10000) / 1000, 1)
            press = weather["main"]["pressure"]

            forecast_summary = "48h forecast unavailable from provider."
            ...
            if forecast:
                forecast_status = getattr(forecast, "status_code", None)
            if forecast and forecast.ok:
                ...
                rain_mm = sum(item.get("rain", {}).get("3h", 0) for item in items if isinstance(item, dict))
                if temps:
                    forecast_summary = (
            ...
            climate = MONTHLY_CLIMATE.get(code, [(20, 50, 7, "Pleasant")] * 12)
            avg_max, rain_mm_month, sun_hrs, *_ = climate[month_idx % 12]
            ...
            return {
                ...
                "provider_diagnostics": self._diag,
            }
        except Exception as exc:
            self._log.debug("OWM error: %s", exc)
            self._set_diag(
                status="exception",
                city=code,
                resolved_city=city_name,
    ...
    def _climate(self, code, city_name, country, month_idx):
        ...
        avg_max, rain_mm, sun_hrs, *_ = table[month_idx % 12]
        months = [
    ...
    def _score_confidence(self, result):
        ...
        return 0.98 if "live" in source else 0.88


def _advisory(temp, desc, month):
    desc_l = desc.lower()
    if temp > 33:
        return f"Very hot ({temp}C). Stay hydrated, avoid midday sun. Factor 50+ sunscreen essential."
    if temp > 27:
        return f"Hot and sunny ({temp}C). Perfect beach weather. Light clothing, high SPF sunscreen."
    if temp > 22:
        return f"Warm and pleasant ({temp}C). Ideal for sightseeing and outdoor activities."
    if temp > 16:
        return f"Mild ({temp}C). Light jacket for evenings. Comfortable for walking tours."
    if temp > 10:
        return f"Cool ({temp}C). Layers recommended. {'Rainy - pack waterproof.' if 'rain' in desc_l else 'Some cloud likely.'}"
...
def _packing(temp, desc):
    ...
    if temp > 27:
        return ["light clothing", "swimwear", "sunscreen SPF50+", "sunglasses", "hat", "flip flops"]
    if temp > 20:
        base = ["summer clothes", "light cardigan", "sunscreen", "comfortable walking shoes"]
        return base + (["travel umbrella"] if rain else [])
    if temp > 14:
        return ["layers", "light jacket", "waterproof", "comfortable shoes", "day bag"]
    return ["warm coat", "thermal layers", "waterproof jacket", "boots", "gloves"]


def _best_time(code):
    times = {
        "DXB": "Nov-Mar (avoid summer heat Jun-Sep, up to 45C)",
        "BKK": "Nov-Mar (dry season; Apr-Oct is monsoon season)",
        "DPS": "Apr-Sep (dry season; avoid Oct-Mar monsoon)",
        "MLE": "Nov-Apr (dry northeast monsoon; best diving and weather)",
        ...
        "SIN": "Feb-Apr (drier; year-round warm)",
        ...
        "VNS": "Nov-Feb (cooler and drier; avoid peak monsoon Jul-Sep)",
    }
